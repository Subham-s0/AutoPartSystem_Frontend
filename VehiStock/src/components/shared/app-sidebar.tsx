...
export function AppSidebar({
  ...
  function isNavigationItemActive(itemTo: string) {
    const [path, hash = ''] = itemTo.split('#')
    ...
    if (hash) {
      ...
      return hash === 'search' && currentHash === ''
    }
        ...
        <div key={section.title}>
          ...
          <ul className="sb-nav">
            {section.items.map((item) => (
              <li key={item.to}>
                <NavLink
                  className={() => (isNavigationItemActive(item.to) ? 'active' : undefined)}
                  end={shouldUseExactMatch(item.to.split('#')[0] ?? item.to)}
                  to={item.to}
                >
                  <item.icon />
                  {item.label}
